...
<cfsilent>
	...
	<div id="svGallery"> 
			...
			<cfoutput>
			<li class="#class#">
			<a href="#createHREFForImage(item.getValue('siteid'),item.getValue('fileid'),item.getValue('fileext'),'large',false)#" title="#HTMLEditFormat(item.getValue('title'))#" rel="shadowbox[gallery]" class="gallery"><img src="#createHREFForImage(item.getValue('siteid'),item.getValue('fileid'),item.getValue('fileext'),'medium')#" alt="#HTMLEditFormat(item.getValue('title'))#"/></a>	 
		 	<!---
<dl>
		 	<dt>
		 	<cfif hasComments>
